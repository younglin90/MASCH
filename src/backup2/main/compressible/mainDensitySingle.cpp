...
int main(int argc, char* argv[]) {
	...
	MASCH_Variables var;
	var.cell = new double*[50];
	
	// mesh.setGeometric();
	// mesh.setCoeffLeastSquare();
	
	// solvers.updateCellEOSfrom_Y(mesh, controls, species);
	// solvers.updateCellTransport(mesh, controls, species);
	
	// SEMO_Mesh_Save save;
	// SEMO_Solvers solver;
	
	// while( control.checkContinueRealTimeStep() ){
		
		// solver.updateOldVar(var.cells, var.faces, var.points);
		
		// solver.fvm(mesh, control, var.cells, var.faces, var.points);
		// solver.dpm(mesh, control, var.cells, var.particles);
		
		// contols.updateRealTime();
