...
double SEMO_Solvers_Builder::calcCompEnergyEq(
	...
	// for(int i=0; i<mesh.cells.size(); ++i){
        ...
        // // A_vals[id] = cTime * ( dHtDT * Rho )*volume/timeStep;
        // // A_vals[id] -= dRhoDT*(
        ...
        // B[step_loc*0 + i_loc] = -(Rho*Ht - oldRho*oldHt)*volume / timeStep + (P - oldP)*volume / timeStep;
		
		
		// // // gravity force terms
        // // B[step_loc*0 + i_loc] += Rho*(
			// // controls.gravityAcceleration[0]*U +
			// // controls.gravityAcceleration[1]*V +
        ...
        // // B[step_loc*0 + i_loc] += volume*(-species[0].sigma * kappa[i] * 
	...
	// for(int i=0; i<mesh.faces.size(); ++i){
        ...
        // // ----------------------------
		// B[step_loc_L*0 + i_loc_L] -= ( RhoF * UnF * HtF * area );
		
		// if(face.getType() == SEMO_Types::INTERNAL_FACE){
			// B[step_loc_R*0 + i_loc_R] += ( RhoF * UnF * HtF * area );
		// }
	...
	// for(auto& boundary : mesh.boundary){
		...
		// if(boundary.neighbProcNo == -1){
			...
			// for(int i=str; i<end; ++i){
				...
				// // ----------------------------
				// B[step_loc*0 + i_loc_L] -= ( RhoF * UnF * HtF * area );
	...
	// for(int i=0; i<mesh.cells.size(); ++i){
		...
		// cell.var[controls.T] +=     relaxT   * resiVar[step_loc*0 + i_loc];
