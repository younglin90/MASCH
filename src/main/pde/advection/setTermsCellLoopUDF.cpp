...
void MASCH_Solver::setTermsCellLoopFunctionsUDF(MASCH_Mesh& mesh, MASCH_Control& controls){
	...
	MASCH_Load load;
	{
		...
		double* cells, double* fields, double* fluxA, double* fluxB) ->int {
			...
			int iter=0;
			fluxA[iter++] = C_dt * vol / dt;
			fluxA[iter++] = 0.0;
			fluxA[iter++] = 0.0;
			fluxA[iter++] = 0.0;
			fluxA[iter++] = 0.0;//C_dt * drhodT * vol / dt;
			for(int j=0; j<nSp-1; ++j){
				fluxA[iter++] = 0.0;//C_dt * cells[id_drhodY[j]] * vol / dt;
			}
			
			fluxA[iter++] = 0.0;//C_dt * drhodp*u * vol / dt - drhodp*gVec_x*vol;
			fluxA[iter++] = C_dt * vol / dt;
			fluxA[iter++] = 0.0;
			fluxA[iter++] = 0.0;
			fluxA[iter++] = 0.0;//C_dt * drhodT*u * vol / dt - drhodT*gVec_x*vol;
			for(int j=0; j<nSp-1; ++j){
				// double drhodY = cells[id_drhodY[j]];
				fluxA[iter++] = 0.0;//C_dt * drhodY*u * vol / dt - drhodY*gVec_x*vol;
			}
			
			fluxA[iter++] = 0.0;//C_dt * drhodp*v * vol / dt - drhodp*gVec_y*vol;
			fluxA[iter++] = 0.0;
			fluxA[iter++] = C_dt * vol / dt;
			fluxA[iter++] = 0.0;
			fluxA[iter++] = 0.0;//C_dt * drhodT*v * vol / dt - drhodT*gVec_y*vol;
			for(int j=0; j<nSp-1; ++j){
				// double drhodY = cells[id_drhodY[j]];
				fluxA[iter++] = 0.0;//C_dt * drhodY*v * vol / dt - drhodY*gVec_y*vol;
			}
			
			fluxA[iter++] = 0.0;//C_dt * drhodp*w * vol / dt - drhodp*gVec_z*vol;
			fluxA[iter++] = 0.0;
			fluxA[iter++] = 0.0;
			fluxA[iter++] = C_dt * vol / dt;
			fluxA[iter++] = 0.0;//C_dt * drhodT*w * vol / dt - drhodT*gVec_z*vol;
			for(int j=0; j<nSp-1; ++j){
				// double drhodY = cells[id_drhodY[j]];
				fluxA[iter++] = 0.0;//C_dt * drhodY*w * vol / dt - drhodY*gVec_z*vol;
			}
			
			fluxA[iter++] = 0.0;//C_dt * (drhodp*Ht+rho*dHtdp-1.0) * vol / dt - 
							// drhodp*(gVec_x*u+gVec_y*v+gVec_z*w)*vol;
			fluxA[iter++] = 0.0;//C_dt * rho*u * vol / dt - rho*gVec_x*vol;
			fluxA[iter++] = 0.0;//C_dt * rho*v * vol / dt - rho*gVec_y*vol;
			fluxA[iter++] = 0.0;//C_dt * rho*w * vol / dt - rho*gVec_z*vol;
			fluxA[iter++] = C_dt * vol / dt;
			for(int j=0; j<nSp-1; ++j){
				// double drhodY = cells[id_drhodY[j]];
				fluxA[iter++] = 0.0;//C_dt * (drhodY*Ht+rho*cells[id_dHtdY[j]]) * vol / dt - 
								// drhodY*(gVec_x*u+gVec_y*v+gVec_z*w)*vol;
			}
			
			for(int i=0; i<nSp-1; ++i){
				double Yi = cells[id_Y[i]];
				fluxA[iter++] = 0.0;//C_dt * drhodp*Yi * vol / dt;
				fluxA[iter++] = 0.0;
				fluxA[iter++] = 0.0;
				fluxA[iter++] = 0.0;
				fluxA[iter++] = 0.0;//C_dt * drhodT*Yi * vol / dt;
				for(int j=0; j<nSp-1; ++j){
					fluxA[iter] = 0.0;//C_dt * cells[id_drhodY[j]]*Yi * vol / dt;
					if(i==j) fluxA[iter] += C_dt * vol / dt;
					++iter;
				}
			}
			...
			// for(int i=0; i<nSp-1; ++i){
				// fluxB[iter++] = -(C_dt_coeff1*Y[i]-C_dt_coeff2*Y_old[i]+C_dt_coeff3*Y_old2[i])*vol/dt;
			// }
