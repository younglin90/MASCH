            ...
            vector<vector<double>>& point_min)->int{
				...
				for(int ii=0; ii<2; ++ii){
                    ...
                    
                    {
                        ...
                        phiL2[5] = max(YR_min[0],min(YR_max[0],phiL2[5]));
                        
                        double tmp_gam = (solver.NVD.*NVD_scheme_i[5])(phiL2[5],phiL1[5],phiR1[5],0.0,0.0,0.0);
                        // double tmp_gam = 0.0;
                        
                        {
                            double tmp_duvwdxyz[3];
                            if(ii==0){
                                // tmp_duvwdxyz[0] = dYdxL[0];
                                // tmp_duvwdxyz[1] = dYdyL[0];
                                // tmp_duvwdxyz[2] = dYdzL[0];
                                tmp_duvwdxyz[0] = 0.5*(dYdxL[0]+dYdxR[0]);
                                tmp_duvwdxyz[1] = 0.5*(dYdyL[0]+dYdyR[0]);
                                tmp_duvwdxyz[2] = 0.5*(dYdzL[0]+dYdzR[0]);
                            }
                            else{
                                // tmp_duvwdxyz[0] = dYdxR[0];
                                // tmp_duvwdxyz[1] = dYdyR[0];
                                // tmp_duvwdxyz[2] = dYdzR[0];
                                tmp_duvwdxyz[0] = 0.5*(dYdxL[0]+dYdxR[0]);
                                tmp_duvwdxyz[1] = 0.5*(dYdyL[0]+dYdyR[0]);
                                tmp_duvwdxyz[2] = 0.5*(dYdzL[0]+dYdzR[0]);
                            }
                            
                            double lim_r = (phiR1[5]-phiL1[5]);
                            if(lim_r!=0.0) lim_r = (phiL1[5]-phiL2[5])/lim_r;
                            
                            double tildeCd = (phiR1[5]-phiL2[5]);
                            if(tildeCd!=0.0) tildeCd = (phiL1[5]-phiL2[5])/tildeCd;
                            ...
                            for(auto& xyz : point_xyz){
                                ...
                                // double tmp_al;
                                // if(tmp_k>=0.0){
                                    // tmp_al = abs(point_max_i[tmp_iter3][3]-phiL1[5])/
                                                // max(abs(tmp_k)*min(1.0,lim_r),1.e-200);
                                // }
                                // else{
                                    // tmp_al = abs(point_min_i[tmp_iter3][3]-phiL1[5])/
                                                // max(abs(tmp_k)*min(1.0,lim_r),1.e-200);
                                // }
                                ...
                                // tmp_k = 0.0;
                                // if(ii==0){
                                    // tmp_k += dYdxL[0]*(xyz[0]-(axyzLR[0]-0.5*xyzLR[0]));
                                    // tmp_k += dYdyL[0]*(xyz[1]-(axyzLR[1]-0.5*xyzLR[1]));
                                    // tmp_k += dYdzL[0]*(xyz[2]-(axyzLR[2]-0.5*xyzLR[2]));
                                // }
                                // else{
                                    // tmp_k += dYdxR[0]*(xyz[0]-(axyzLR[0]+0.5*xyzLR[0]));
                                    // tmp_k += dYdyR[0]*(xyz[1]-(axyzLR[1]+0.5*xyzLR[1]));
                                    // tmp_k += dYdzR[0]*(xyz[2]-(axyzLR[2]+0.5*xyzLR[2]));
                                // }
                                // double tmp_al = 1.0;
                                // if(tmp_k>0.0){
                                    // tmp_al = (point_max_i[tmp_iter3][3]-phiL1[5])/tmp_k;
                                // }
                                // else if(tmp_k<0.0){
                                    // tmp_al = (point_min_i[tmp_iter3][3]-phiL1[5])/tmp_k;
                                // }
                                // alpha_MLP = min(alpha_MLP,tmp_al);
                                ...
                                if(Delta_minus>0.0){
                                    double Delta_plus = point_max_i[tmp_iter3][3] - phiL1[5];
                                    ...
                                    double Delta_plus = point_min_i[tmp_iter3][3] - phiL1[5];
                                    tmp_al = 1.0/Delta_minus*
                                        ((Delta_plus*Delta_plus+eta*eta)*
                                        Delta_minus+2.0*Delta_minus*Delta_minus*Delta_plus)/
                                        (Delta_plus*Delta_plus+2.0*Delta_minus*Delta_minus+
                        ...
                        // phiL1[5] = tmp_gam*phiL1[5] + (1.0-tmp_gam)*phiR1[5];
                        ...
                        
                        {
                            ...
                            phiL1[5] = phiL1[5] + (1.0-tmp_gam)*tmp_k;
                        }
					...
					// {
						...
						// double coDD = 0.000000000000001; //1.0*max(1.0*dt/dt_tmp_L,1.0*dt/dt_tmp_R);
                        // // coDD = min(coDD,1.0);
