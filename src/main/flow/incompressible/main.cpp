...
int main(int argc, char* argv[]) {
	...
	vector<string> s_iter = load.extractVector(controls.fvSolutionMap["segregated.nCorrectors"]);
	int maxIterOuter = stoi(s_iter[0]);
	while( 
	...
	!bool_resi_isnan
		...
		for(int iterOuter=0; iterOuter<maxIterOuter; ++iterOuter){
			// 레지듀얼 초기화
	...
	{
		save.fvmFiles("./save/potential/", rank, mesh, controls, var);
	}
	MPI_Barrier(MPI_COMM_WORLD);
