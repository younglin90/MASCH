...
void MASCH_Solver::setDPMFunctionsUDF(MASCH_Mesh& mesh, MASCH_Control& controls, MASCH_Variables& var){
	...
	solver.calcDPM_parcelLoop.push_back(
        ...
        minBreakupChildVolume,maxBreakupNumberOfParcel, nSp](
		int icell, double* cells, double* fields, double* parcels, double* fluxA, double* fluxB) ->int {
			...
			if(Red>1.0) {
                ...
                double coeff = Cd * rho * PI * d_p*d_p * urelAbs / 8.0;
                force_x = coeff * urel;
                force_y = coeff * vrel;
                force_z = coeff * wrel;
                ...
                double mass = 4.0/3.0*PI*0.125*d_p*d_p*d_p * rho_p;
                ...
                fluxB[1] -= force_x * N_p * dt_p/dt;
                fluxB[2] -= force_y * N_p * dt_p/dt;
                fluxB[3] -= force_z * N_p * dt_p/dt;
                fluxB[4] -= energy * N_p * dt_p/dt;
                ...
                fluxA[iter+1] += (coeff) * N_p * dt_p/dt;
                iter += (5+nSp-1);
                
                fluxA[iter+2] += (coeff) * N_p * dt_p/dt;
                iter += (5+nSp-1);
                
                fluxA[iter+3] += (coeff) * N_p * dt_p/dt;
                iter += (5+nSp-1);
                
                fluxA[iter+1] += (coeff) * parcels[id_u_p] * N_p * dt_p/dt;
                fluxA[iter+2] += (coeff) * parcels[id_v_p] * N_p * dt_p/dt;
                fluxA[iter+3] += (coeff) * parcels[id_w_p] * N_p * dt_p/dt;
    ...
    // double maxBreakupNumberOfParcel = stod(controls.controlParcelsMap["maxBreakupNumberOfParcel"]);
            ...
            if(d_p>minBreakupDiameter && N_p < maxBreakupNumberOfParcel){
                ...
                if (r_child_KH > r_p) {
                   double r1 = PI*urelAbs/(2.0*omegaKH);
                ...
                if(boolBreakupWork==true){
                    ...
                    { 
                        ...
                        ran_theta = ran1;
                        double phi = ran_theta * 2.0 * PI; // double_pi = 2 * pi
