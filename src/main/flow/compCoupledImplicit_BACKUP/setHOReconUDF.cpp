...
void MASCH_Solver::setHOReconFunctionsUDF(MASCH_Mesh& mesh, MASCH_Control& controls){
	...
	
	{
		calcHO_FaceVal.push_back(
			...
			// // UnF = chat*MLPL + chat*MRMR - 0.5*phi_c/rhohat/chat*(pR-pL);
			// // double mdot = rhoL*chat*MLPL + rhoR*chat*MRMR - 0.5*phi_c/chat*(pR-pL);
			// double mdot = rhoL*chat*MLPL + rhoR*chat*MRMR - 0.5*phi_c/chat*(pR-pL);
			// UnF = mdot/rhoL;
			// if( mdot < 0.0 ) {
				// UnF = mdot/rhoR;
			// }
			// // UnF	= UnF - 0.5*phi_c/(0.5*(rhoL+rhoR))/chat*(pR-pL);
			...
			// // faces[id_pF] = PLR;
				...
				faces[id_uF] = solver.NVD.vanLeer(phiL2[0],phiL1[0],phiR1[0]);
				faces[id_vF] = solver.NVD.vanLeer(phiL2[1],phiL1[1],phiR1[1]);
				faces[id_wF] = solver.NVD.vanLeer(phiL2[2],phiL1[2],phiR1[2]);
				faces[id_TF] = solver.NVD.vanLeer(phiL2[3],phiL1[3],phiR1[3]);
