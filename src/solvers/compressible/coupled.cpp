...
double SEMO_Solvers_Builder::calcCompCoupledEq(
	...
	// vector<double> P_recv, U_recv, V_recv, W_recv, T_recv, MF0_recv;
	// if(size>1){
		// // processor faces
		// proc_num = 0;
		// vector<double> P_send, U_send, V_send, W_send, T_send, MF0_send;
		// for(int i=0; i<mesh.faces.size(); ++i){
			// auto& face = mesh.faces[i];
			...
			// if(face.getType() == SEMO_Types::PROCESSOR_FACE){
				...
				// T_send.push_back(mesh.cells[face.owner].var[controls.T]);
				// MF0_send.push_back(mesh.cells[face.owner].var[controls.MF[0]]);
				// ++proc_num;
			// }
		// }
		// // SEMO_MPI_Builder mpi;
	...
	// for(int i=0; i<mesh.cells.size(); ++i){
        ...
        // A_vals[id] = cTime * ( dRhoDP )*volume/timeStep;
        ...
        // id = step_loc*(B_n*0+4) + i_loc; i_glo = str_glo + step_loc*0 + i_loc; j_glo = str_glo + step_loc*4 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDT )*volume/timeStep;
        
        // id = step_loc*(B_n*0+5) + i_loc; i_glo = str_glo + step_loc*0 + i_loc; j_glo = str_glo + step_loc*5 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDMF[0] )*volume/timeStep;

        // // x-momentum
        // id = step_loc*(B_n*1+0) + i_loc; i_glo = str_glo + step_loc*1 + i_loc; j_glo = str_glo + step_loc*0 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDP * U )*volume/timeStep;
        // A_vals[id] -= dRhoDP*controls.gravityAcceleration[0]*volume;
        ...
        // id = step_loc*(B_n*1+4) + i_loc; i_glo = str_glo + step_loc*1 + i_loc; j_glo = str_glo + step_loc*4 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDT * U )*volume/timeStep;
        // A_vals[id] -= dRhoDT*controls.gravityAcceleration[0]*volume;
        
        // id = step_loc*(B_n*1+5) + i_loc; i_glo = str_glo + step_loc*1 + i_loc; j_glo = str_glo + step_loc*5 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDMF[0] * U )*volume/timeStep;
        // A_vals[id] -= dRhoDMF[0]*controls.gravityAcceleration[0]*volume;

        // // y-momentum
        // id = step_loc*(B_n*2+0) + i_loc; i_glo = str_glo + step_loc*2 + i_loc; j_glo = str_glo + step_loc*0 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDP * V )*volume/timeStep;
        // A_vals[id] -= dRhoDP*controls.gravityAcceleration[1]*volume;
        ...
        // id = step_loc*(B_n*2+4) + i_loc; i_glo = str_glo + step_loc*2 + i_loc; j_glo = str_glo + step_loc*4 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDT * V )*volume/timeStep;
        // A_vals[id] -= dRhoDT*controls.gravityAcceleration[1]*volume;
        
        // id = step_loc*(B_n*2+5) + i_loc; i_glo = str_glo + step_loc*2 + i_loc; j_glo = str_glo + step_loc*5 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDMF[0] * V )*volume/timeStep;
        // A_vals[id] -= dRhoDMF[0]*controls.gravityAcceleration[1]*volume;
		

        // // z-momentum
        // id = step_loc*(B_n*3+0) + i_loc; i_glo = str_glo + step_loc*3 + i_loc; j_glo = str_glo + step_loc*0 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDP * W )*volume/timeStep;
        // A_vals[id] -= dRhoDP*controls.gravityAcceleration[2]*volume;
        ...
        // id = step_loc*(B_n*3+4) + i_loc; i_glo = str_glo + step_loc*3 + i_loc; j_glo = str_glo + step_loc*4 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDT * W )*volume/timeStep;
        // A_vals[id] -= dRhoDT*controls.gravityAcceleration[2]*volume;
        
        // id = step_loc*(B_n*3+5) + i_loc; i_glo = str_glo + step_loc*3 + i_loc; j_glo = str_glo + step_loc*5 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDMF[0] * W )*volume/timeStep;
        // A_vals[id] -= dRhoDMF[0]*controls.gravityAcceleration[2]*volume;
		

        // // energy
        // id = step_loc*(B_n*4+0) + i_loc; i_glo = str_glo + step_loc*4 + i_loc; j_glo = str_glo + step_loc*0 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDP * Ht + dHtDP * Rho - 1.0 )*volume/timeStep;
        // A_vals[id] -= dRhoDP*(
						// controls.gravityAcceleration[0]*U +
						// controls.gravityAcceleration[1]*V +
						// controls.gravityAcceleration[2]*W
						// )*volume;

        // id = step_loc*(B_n*4+1) + i_loc; i_glo = str_glo + step_loc*4 + i_loc; j_glo = str_glo + step_loc*1 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( Rho * U )*volume/timeStep;
        // A_vals[id] -= Rho*(controls.gravityAcceleration[0])*volume;
		
        // id = step_loc*(B_n*4+2) + i_loc; i_glo = str_glo + step_loc*4 + i_loc; j_glo = str_glo + step_loc*2 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( Rho * V )*volume/timeStep;
        // A_vals[id] -= Rho*(controls.gravityAcceleration[1])*volume;
        
        // id = step_loc*(B_n*4+3) + i_loc; i_glo = str_glo + step_loc*4 + i_loc; j_glo = str_glo + step_loc*3 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( Rho * W )*volume/timeStep;
        // A_vals[id] -= Rho*(controls.gravityAcceleration[2])*volume;
        
        // id = step_loc*(B_n*4+4) + i_loc; i_glo = str_glo + step_loc*4 + i_loc; j_glo = str_glo + step_loc*4 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDT * Ht + dHtDT * Rho )*volume/timeStep;
        // A_vals[id] -= dRhoDT*(
						// controls.gravityAcceleration[0]*U +
						// controls.gravityAcceleration[1]*V +
						// controls.gravityAcceleration[2]*W
        ...
        // id = step_loc*(B_n*4+5) + i_loc; i_glo = str_glo + step_loc*4 + i_loc; j_glo = str_glo + step_loc*5 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDMF[0] * Ht + dHtDMF[0] * Rho )*volume/timeStep;
		

        // // mass-fraction
        // id = step_loc*(B_n*5+0) + i_loc; i_glo = str_glo + step_loc*5 + i_loc; j_glo = str_glo + step_loc*0 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDP * MF[0] )*volume/timeStep;

        // id = step_loc*(B_n*5+1) + i_loc; i_glo = str_glo + step_loc*5 + i_loc; j_glo = str_glo + step_loc*1 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
		
        // id = step_loc*(B_n*5+2) + i_loc; i_glo = str_glo + step_loc*5 + i_loc; j_glo = str_glo + step_loc*2 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        
        // id = step_loc*(B_n*5+3) + i_loc; i_glo = str_glo + step_loc*5 + i_loc; j_glo = str_glo + step_loc*3 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        
        // id = step_loc*(B_n*5+4) + i_loc; i_glo = str_glo + step_loc*5 + i_loc; j_glo = str_glo + step_loc*4 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDT * MF[0] )*volume/timeStep;
        
        // id = step_loc*(B_n*5+5) + i_loc; i_glo = str_glo + step_loc*5 + i_loc; j_glo = str_glo + step_loc*5 + i_loc;
        // A_rows[id] = i_glo; A_cols[id] = j_glo;
        // A_vals[id] = cTime * ( dRhoDMF[0] * MF[0] + Rho )*volume/timeStep;



		
		// // // first order time integration
        // // B[step_loc*0 + i_loc] = -(Rho - oldRho)*volume / timeStep;
        // // B[step_loc*1 + i_loc] = -(Rho*U - oldRho*oldU)*volume / timeStep;
        // // B[step_loc*2 + i_loc] = -(Rho*V - oldRho*oldV)*volume / timeStep;
        // // B[step_loc*3 + i_loc] = -(Rho*W - oldRho*oldW)*volume / timeStep;
        // // B[step_loc*4 + i_loc] = -(Rho*Ht - oldRho*oldHt)*volume / timeStep + (P - oldP)*volume / timeStep;
        // // B[step_loc*5 + i_loc] = -(Rho*MF[0] - oldRho*oldMF[0])*volume / timeStep;
		
		
		
		// // // second order time integration
        // // B[step_loc*0 + i_loc] = -(1.5*Rho - 2.0*oldRho + 0.5*old2Rho)*volume / timeStep;
        // // B[step_loc*1 + i_loc] = -(1.5*Rho*U - 2.0*oldRho*oldU + 0.5*old2Rho*old2U)*volume / timeStep;
        // // B[step_loc*2 + i_loc] = -(1.5*Rho*V - 2.0*oldRho*oldV + 0.5*old2Rho*old2V)*volume / timeStep;
        // // B[step_loc*3 + i_loc] = -(1.5*Rho*W - 2.0*oldRho*oldW + 0.5*old2Rho*old2W)*volume / timeStep;
        // // B[step_loc*4 + i_loc] = -(1.5*Rho*Ht - 2.0*oldRho*oldHt + 0.5*old2Rho*old2Ht)*volume / timeStep + 
								 // // (1.5*P - 2.0*oldP + 0.5*old2P)*volume / timeStep;
        // // B[step_loc*5 + i_loc] = -(1.5*Rho*MF[0] - 2.0*oldRho*oldMF[0] + 0.5*old2Rho*old2MF[0])*volume / timeStep;
		
		
		
		// // bounded second order time integration
		// vector<double> conservative_n(B_n,0.0);
		// vector<double> conservative_nm1(B_n,0.0);
		// vector<double> conservative_nm2(B_n,0.0);
		// conservative_n[0] = Rho;       conservative_nm1[0] = oldRho;            conservative_nm2[0] = old2Rho;
		// conservative_n[1] = Rho*U;     conservative_nm1[1] = oldRho*oldU;       conservative_nm2[1] = old2Rho*old2U;
		// conservative_n[2] = Rho*V;     conservative_nm1[2] = oldRho*oldV;       conservative_nm2[2] = old2Rho*old2V;
		// conservative_n[3] = Rho*W;     conservative_nm1[3] = oldRho*oldW;       conservative_nm2[3] = old2Rho*old2W;
		// conservative_n[4] = Rho*Ht-P;  conservative_nm1[4] = oldRho*oldHt-oldP; conservative_nm2[4] = old2Rho*old2Ht-old2P;
		// conservative_n[5] = Rho*MF[0]; conservative_nm1[5] = oldRho*oldMF[0];   conservative_nm2[5] = old2Rho*old2MF[0];
		
		// double beta_phi = 1.0;
		// for(int ii=0; ii<B_n; ++ii){
			// double phi_C = conservative_nm1[ii];
			// double phi_D = conservative_n[ii];
			// double phi_U = conservative_nm2[ii];
			// double phi_SOU = ( phi_C - phi_U ) / ( phi_D - phi_U + 1.d-200 );
		
			// if(0.0 < phi_SOU && phi_SOU < 1.0){
				// beta_phi = min(beta_phi, 0.0);
			// }
			// else{
				// beta_phi = min(beta_phi, 1.0);
			// }
		// }
		...
		// for(int ii=0; ii<B_n; ++ii){
			...
			// double cons_np12 = conservative_n[ii] + beta_phi * coeff_old1 * (conservative_n[ii]-conservative_nm1[ii]);
			// double cons_nm12 = conservative_nm1[ii] + beta_phi * coeff_old2 * (conservative_nm1[ii]-conservative_nm2[ii]);
			// B[step_loc*ii + i_loc] = -( cons_np12 - cons_nm12 )*volume / timeStep;
		// }
        ...
        // B[step_loc*4 + i_loc] += Rho*(
			// controls.gravityAcceleration[0]*U +
			// controls.gravityAcceleration[1]*V +
			// controls.gravityAcceleration[2]*W )*volume;


		// // surface tension force terms
        // B[step_loc*1 + i_loc] += volume*(-species[0].sigma * kappa[i] * gradAi[i][0]);
        // B[step_loc*2 + i_loc] += volume*(-species[0].sigma * kappa[i] * gradAi[i][1]);
        // B[step_loc*3 + i_loc] += volume*(-species[0].sigma * kappa[i] * gradAi[i][2]);
        // B[step_loc*4 + i_loc] += volume*(-species[0].sigma * kappa[i] * 
			// ( gradAi[i][0]*U + gradAi[i][1]*V + gradAi[i][2]*W ) );
	...
	// for(int i=0; i<mesh.faces.size(); ++i){
		...
		// double Rho_star = 1.0 / (wCRhoL/RhoL + wCRhoR/RhoR);
		...
		// // pressure correction (cell->face, with non-orthogonal) 
		// double UnF_RC = 0.0;
		// UnF_RC -= alpha * tmp1*(orgPR-orgPL)/dPN;
		// for(int ii=0; ii<3; ++ii){
			// UnF_RC += alpha * tmp1 * Rho_star * wCRhoL/RhoL*gradPL[ii]*face.unitNomalsPN[ii];
			// UnF_RC += alpha * tmp1 * Rho_star * wCRhoR/RhoR*gradPR[ii]*face.unitNomalsPN[ii];
		// }
		// // pressure correction (skewness)
		// for(int ii=0; ii<3; ++ii){
			// UnF_RC -= alpha * tmp1*(gradPR[ii]-gradPL[ii])*face.vecSkewness[ii]/dPN;
		// }
		...
		// // gradVFL[0] = gradAi[face.owner][0]; gradVFL[1] = gradAi[face.owner][1]; gradVFL[2] = gradAi[face.owner][2];
		// // vector<double> gradVFR(3,0.0);
		
		// // if(face.getType() == SEMO_Types::INTERNAL_FACE){
			// // orgVFR = mesh.cells[face.neighbour].var[controls.VF[0]];
			// // kappaR = kappa[face.neighbour];
			// // gradVFR[0] = gradAi[face.neighbour][0]; gradVFR[1] = gradAi[face.neighbour][1]; gradVFR[2] = gradAi[face.neighbour][2];
		// // }
		// // else if(face.getType() == SEMO_Types::PROCESSOR_FACE){
			// // orgVFR = face.varR[controls.fVF[0]];
			// // kappaR = kappa_recv[proc_num];
			// // gradVFR[0] = gradAix_recv[proc_num]; gradVFR[1] = gradAiy_recv[proc_num]; gradVFR[2] = gradAiz_recv[proc_num];
		// // }
		
		// // for(int ii=0; ii<3; ++ii){
			// // UnF_RC += alpha * tmp1 * species[0].sigma * Rho_star * wCPL/RhoL* kappaL*gradVFL[ii]*face.unitNomalsPN[ii];
			// // UnF_RC += alpha * tmp1 * species[0].sigma * Rho_star * wCPR/RhoR* kappaR*gradVFR[ii]*face.unitNomalsPN[ii];
		// // }
		// // UnF_RC -= alpha * tmp1 * species[0].sigma * 0.5*(kappaL + kappaR) * (orgVFR-orgVFL)/dPN;
        ...
        // // UnF = wCL*UnL + wCR*UnR + UnF_RC - 0.5*chi/rhohat/Cbar*(orgPR-orgPL);
        // // UnF = wCL*UnL + wCR*UnR + UnF_RC;
		...
		// // double diffPDU = 0.5*Mdash*PLP*PRM*0.5/Cbar;
		// double diffPDU = 0.0;
		...
		// for(int ii=0; ii<3; ++ii){
			...
			// dUdnF += wCL * gradUL[ii] * tvec;
			// dUdnF += wCR * gradUR[ii] * tvec;
			
			// dVdnF += wCL * gradVL[ii] * tvec;
			// dVdnF += wCR * gradVR[ii] * tvec;
			
			// dWdnF += wCL * gradWL[ii] * tvec;
			// dWdnF += wCR * gradWR[ii] * tvec;
			
			// // face gradients, skewness
			// dUdnF += alpha * (gradUR[ii]-gradUL[ii])*face.vecSkewness[ii]/dPN;
			
			// dVdnF += alpha * (gradVR[ii]-gradVL[ii])*face.vecSkewness[ii]/dPN;
			
			// dWdnF += alpha * (gradWR[ii]-gradWL[ii])*face.vecSkewness[ii]/dPN;
		// }
		// // face gradients
		...
		// // // Ref : denner 학위 논문
		// // double txx = dUdxyzF[0] + dVdxyzF[0] + dWdxyzF[0] - 2.0/3.0 * dUdxyzF[0];
		// // double txy = - 2.0/3.0 * dVdxyzF[1];
		// // double txz = - 2.0/3.0 * dWdxyzF[2];
		// // double tyx = - 2.0/3.0 * dUdxyzF[0];
		// // double tyy = dUdxyzF[1] + dVdxyzF[1] + dWdxyzF[1] - 2.0/3.0 * dVdxyzF[1];
		// // double tyz = - 2.0/3.0 * dWdxyzF[2];
		// // double tzx = - 2.0/3.0 * dUdxyzF[0];
		// // double tzy = - 2.0/3.0 * dVdxyzF[1];
		// // double tzz = dUdxyzF[2] + dVdxyzF[2] + dWdxyzF[2] - 2.0/3.0 * dWdxyzF[2];
		
		
		// double diff_viscous = muF * alpha / dPN;
		// // double diff_viscous = 0.0;
		
		// vector<double> viscous_flux(6,0.0);
		// viscous_flux[1] = muF * (txx*nvec[0] + txy*nvec[1] + txz*nvec[2] + dUdnF) * area;
		// // flux[1] -= nx * 2.0/3.0 * rhoF * tkei;
		// viscous_flux[2] = muF * (tyx*nvec[0] + tyy*nvec[1] + tyz*nvec[2] + dVdnF) * area;
		// // flux[2] -= ny * 2.0/3.0 * rhoF * tkei;
		// viscous_flux[3] = muF * (tzx*nvec[0] + tzy*nvec[1] + tzz*nvec[2] + dWdnF) * area;
		...
		// // // viscous
		// // viscous_flux[1] = alpha * muF*(orgUR-orgUL)/dPN*area;
		// // viscous_flux[2] = alpha * muF*(orgVR-orgVL)/dPN*area;
		// // viscous_flux[3] = alpha * muF*(orgWR-orgWL)/dPN*area;
		
		// // // non-orthogonal
		// // for(int ii=0; ii<3; ++ii){
			// // viscous_flux[1] += ( muF*(wCL*gradUL[ii] + wCR*gradUR[ii])*(nvec[ii] - alpha*face.unitNomalsPN[ii])*area ); 
			// // viscous_flux[2] += ( muF*(wCL*gradVL[ii] + wCR*gradVR[ii])*(nvec[ii] - alpha*face.unitNomalsPN[ii])*area ); 
			// // viscous_flux[3] += ( muF*(wCL*gradWL[ii] + wCR*gradWR[ii])*(nvec[ii] - alpha*face.unitNomalsPN[ii])*area ); 
		// // }
		
		// // diff_viscous = alpha * muF/dPN;
		
		
		

		// // high-order terms
		// double RhoF_HO = weightL * face.varL[controls.fRho_HO] + weightR * face.varR[controls.fRho_HO];
		// double HtF_HO  = weightL * face.varL[controls.fHt_HO] + weightR * face.varR[controls.fHt_HO];
		// // double HtF_HO  = HtF;
		// vector<double> MFF_HO;
		// MFF_HO.push_back( weightL * face.varL[controls.fMF_HO[0]] + weightR * face.varR[controls.fMF_HO[0]] );
		// // MFF_HO.push_back( MFF[0] );
		...
		// A_vals[id] += ( weightL * dRhoDPL * UnF * area + RhoF * diffDP * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDPR * UnF * area - RhoF * diffDP * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// A_vals[id] -= ( weightR * dRhoDPR * UnF * area - RhoF * diffDP * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDPL * UnF * area + RhoF * diffDP * area ));
        ...
        // A_vals[id] += ( RhoF * WUL * nvec[0] * area );
        // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
        // A_vals.push_back(( RhoF * WUR * nvec[0] * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// A_vals[id] -= ( RhoF * WUR * nvec[0] * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[0] * area ));
        ...
        // A_vals[id] += ( RhoF * WUL * nvec[1] * area );
        // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
        // A_vals.push_back(( RhoF * WUR * nvec[1] * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// A_vals[id] -= ( RhoF * WUR * nvec[1] * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[1] * area ));
        ...
        // A_vals[id] += ( RhoF * WUL * nvec[2] * area );
        // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
        // A_vals.push_back(( RhoF * WUR * nvec[2] * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// A_vals[id] -= ( RhoF * WUR * nvec[2] * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[2] * area ));
        ...
        // // T'
        // id = step_loc_L*(B_n*0+4) + i_loc_L; i_glo = str_glo_L + step_loc_L*0 + i_loc_L; j_glo = str_glo_R + step_loc_R*4 + i_loc_R;
        // A_vals[id] += ( weightL * dRhoDTL * UnF * area );
        // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
        // A_vals.push_back(( weightR * dRhoDTR * UnF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*0+4) + i_loc_R; i_glo = str_glo_R + step_loc_R*0 + i_loc_R; j_glo = str_glo_L + step_loc_L*4 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDTR * UnF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDTL * UnF * area ));
        ...
        // // MF[0]'
        // id = step_loc_L*(B_n*0+5) + i_loc_L; i_glo = str_glo_L + step_loc_L*0 + i_loc_L; j_glo = str_glo_R + step_loc_R*5 + i_loc_R;
        // A_vals[id] += ( weightL * dRhoDMFL[0] * UnF * area );
        // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
        // A_vals.push_back(( weightR * dRhoDMFR[0] * UnF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*0+5) + i_loc_R; i_glo = str_glo_R + step_loc_R*0 + i_loc_R; j_glo = str_glo_L + step_loc_L*5 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDMFR[0] * UnF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDMFL[0] * UnF * area ));
        ...
        // A_vals[id] += ( weightL * dRhoDPL * UnF * UF * area + WPL * nvec[0] * area + RhoF * diffDP * UF * area );
        // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
        // A_vals.push_back(( weightR * dRhoDPR * UnF * UF * area + WPR * nvec[0] * area - RhoF * diffDP * UF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// A_vals[id] -= ( weightR * dRhoDPR * UnF * UF * area + WPR * nvec[0] * area - RhoF * diffDP * UF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDPL * UnF * UF * area + WPL * nvec[0] * area + RhoF * diffDP * UF * area ));
		...
		// A_vals[id] += ( RhoF * WUL * nvec[0] * UF * area + RhoF * UnF * weightL * area + diff_viscous * area + 
						// diffPDU*nvec[0] * nvec[0] * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( RhoF * WUR * nvec[0] * UF * area + RhoF * UnF * weightR * area - diff_viscous * area - 
						// diffPDU*nvec[0] * nvec[0] * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// A_vals[id] -= ( RhoF * WUR * nvec[0] * UF * area + RhoF * UnF * weightR * area - diff_viscous * area - 
						// diffPDU*nvec[0] * nvec[0] * area  );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[0] * UF * area + RhoF * UnF * weightL * area + diff_viscous * area + 
						// diffPDU*nvec[0] * nvec[0] * area  ));
		...
		// // A_vals[id] += ( RhoF * WUL * nvec[1] * UF * area +
						// // diffPDU*nvec[1] * nvec[0] * area
						// // );
		// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// // A_vals.push_back(( RhoF * WUR * nvec[1] * UF * area -
						// // diffPDU*nvec[1] * nvec[0] * area
						// // ));
		...
		// // if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// // A_vals[id] -= ( RhoF * WUR * nvec[1] * UF * area -
						// // diffPDU*nvec[1] * nvec[0] * area
						// // );
			// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// // A_vals.push_back(-( RhoF * WUL * nvec[1] * UF * area +
						// // diffPDU*nvec[1] * nvec[0] * area
						// // ));
		...
		// // A_vals[id] += ( RhoF * WUL * nvec[2] * UF * area +
						// // diffPDU*nvec[2] * nvec[0] * area
						// // );
		// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// // A_vals.push_back(( RhoF * WUR * nvec[2] * UF * area -
						// // diffPDU*nvec[2] * nvec[0] * area
						// // ));
		...
		// // if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// // A_vals[id] -= ( RhoF * WUR * nvec[2] * UF * area -
						// // diffPDU*nvec[2] * nvec[0] * area
						// // );
			// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// // A_vals.push_back(-( RhoF * WUL * nvec[2] * UF * area +
						// // diffPDU*nvec[2] * nvec[0] * area
						// // ));
        ...
        // // T'
		// id = step_loc_L*(B_n*1+4) + i_loc_L; i_glo = str_glo_L + step_loc_L*1 + i_loc_L; j_glo = str_glo_R + step_loc_R*4 + i_loc_R;
		// A_vals[id] += ( weightL * dRhoDTL * UnF * UF * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDTR * UnF * UF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*1+4) + i_loc_R; i_glo = str_glo_R + step_loc_R*1 + i_loc_R; j_glo = str_glo_L + step_loc_L*4 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDTR * UnF * UF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDTL * UnF * UF * area ));
        ...
        // // MF[0]'
		// id = step_loc_L*(B_n*1+5) + i_loc_L; i_glo = str_glo_L + step_loc_L*1 + i_loc_L; j_glo = str_glo_R + step_loc_R*5 + i_loc_R;
		// A_vals[id] += ( weightL * dRhoDMFL[0] * UnF * UF * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDMFR[0] * UnF * UF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*1+5) + i_loc_R; i_glo = str_glo_R + step_loc_R*1 + i_loc_R; j_glo = str_glo_L + step_loc_L*5 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDMFR[0] * UnF * UF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDMFL[0] * UnF * UF * area ));
        ...
        // A_vals[id] += ( weightL * dRhoDPL * UnF * VF * area + WPL * nvec[1] * area + RhoF * diffDP * VF * area );
        // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
        // A_vals.push_back(( weightR * dRhoDPR * UnF * VF * area + WPR * nvec[1] * area - RhoF * diffDP * VF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// A_vals[id] -= ( weightR * dRhoDPR * UnF * VF * area + WPR * nvec[1] * area - RhoF * diffDP * VF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDPL * UnF * VF * area + WPL * nvec[1] * area + RhoF * diffDP * VF * area ));
		...
		// // A_vals[id] += ( RhoF * WUL * nvec[0] * VF * area +
						// // diffPDU*nvec[0] * nvec[1] * area
						// // );
		// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// // A_vals.push_back(( RhoF * WUR * nvec[0] * VF * area -
						// // diffPDU*nvec[0] * nvec[1] * area
						// // ));
		...
		// // if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// // A_vals[id] -= ( RhoF * WUR * nvec[0] * VF * area -
						// // diffPDU*nvec[0] * nvec[1] * area
						// // );
			// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// // A_vals.push_back(-( RhoF * WUL * nvec[0] * VF * area +
						// // diffPDU*nvec[0] * nvec[1] * area
						// // ));
		...
		// A_vals[id] += ( RhoF * WUL * nvec[1] * VF * area + RhoF * UnF * weightL * area + diff_viscous * area +
						// diffPDU*nvec[1] * nvec[1] * area
						// );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( RhoF * WUR * nvec[1] * VF * area + RhoF * UnF * weightR * area - diff_viscous * area -
						// diffPDU*nvec[1] * nvec[1] * area
						// ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// A_vals[id] -= ( RhoF * WUR * nvec[1] * VF * area + RhoF * UnF * weightR * area - diff_viscous * area -
						// diffPDU*nvec[1] * nvec[1] * area
						// );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[1] * VF * area + RhoF * UnF * weightL * area + diff_viscous * area +
						// diffPDU*nvec[1] * nvec[1] * area
						// ));
		...
		// // A_vals[id] += ( RhoF * WUL * nvec[2] * VF * area +
						// // diffPDU*nvec[2] * nvec[1] * area
						// // );
		// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// // A_vals.push_back(( RhoF * WUR * nvec[2] * VF * area -
						// // diffPDU*nvec[2] * nvec[1] * area
						// // ));
		...
		// // if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// // A_vals[id] -= ( RhoF * WUR * nvec[2] * VF * area -
						// // diffPDU*nvec[2] * nvec[1] * area
						// // );
			// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// // A_vals.push_back(-( RhoF * WUL * nvec[2] * VF * area +
						// // diffPDU*nvec[2] * nvec[1] * area
						// // ));
        ...
        // // T'
		// id = step_loc_L*(B_n*2+4) + i_loc_L; i_glo = str_glo_L + step_loc_L*2 + i_loc_L; j_glo = str_glo_R + step_loc_R*4 + i_loc_R;
		// A_vals[id] += ( weightL * dRhoDTL * UnF * VF * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDTR * UnF * VF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*2+4) + i_loc_R; i_glo = str_glo_R + step_loc_R*2 + i_loc_R; j_glo = str_glo_L + step_loc_L*4 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDTR * UnF * VF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDTL * UnF * VF * area ));
        ...
        // // MF[0]'
		// id = step_loc_L*(B_n*2+5) + i_loc_L; i_glo = str_glo_L + step_loc_L*2 + i_loc_L; j_glo = str_glo_R + step_loc_R*5 + i_loc_R;
		// A_vals[id] += ( weightL * dRhoDMFL[0] * UnF * VF * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDMFR[0] * UnF * VF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*2+5) + i_loc_R; i_glo = str_glo_R + step_loc_R*2 + i_loc_R; j_glo = str_glo_L + step_loc_L*5 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDMFR[0] * UnF * VF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDMFL[0] * UnF * VF * area ));
        ...
        // A_vals[id] += ( weightL * dRhoDPL * UnF * WF * area + WPL * nvec[2] * area + RhoF * diffDP * WF * area );
        // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
        // A_vals.push_back(( weightR * dRhoDPR * UnF * WF * area + WPR * nvec[2] * area - RhoF * diffDP * WF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// A_vals[id] -= ( weightR * dRhoDPR * UnF * WF * area + WPR * nvec[2] * area - RhoF * diffDP * WF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDPL * UnF * WF * area + WPL * nvec[2] * area + RhoF * diffDP * WF * area ));
		...
		// // A_vals[id] += ( RhoF * WUL * nvec[0] * WF * area +
						// // diffPDU*nvec[0] * nvec[2] * area
						// // );
		// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// // A_vals.push_back(( RhoF * WUR * nvec[0] * WF * area -
						// // diffPDU*nvec[0] * nvec[2] * area
						// // ));
		...
		// // if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// // A_vals[id] -= ( RhoF * WUR * nvec[0] * WF * area -
						// // diffPDU*nvec[0] * nvec[2] * area
						// // );
			// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// // A_vals.push_back(-( RhoF * WUL * nvec[0] * WF * area +
						// // diffPDU*nvec[0] * nvec[2] * area
						// // ));
		...
		// // A_vals[id] += ( RhoF * WUL * nvec[1] * WF * area +
						// // diffPDU*nvec[1] * nvec[2] * area
						// // );
		// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// // A_vals.push_back(( RhoF * WUR * nvec[1] * WF * area -
						// // diffPDU*nvec[1] * nvec[2] * area
						// // ));
		...
		// // if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// // A_vals[id] -= ( RhoF * WUR * nvec[1] * WF * area -
						// // diffPDU*nvec[1] * nvec[2] * area
						// // );
			// // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// // A_vals.push_back(-( RhoF * WUL * nvec[1] * WF * area +
						// // diffPDU*nvec[1] * nvec[2] * area
						// // ));
		...
		// A_vals[id] += ( RhoF * WUL * nvec[2] * WF * area + RhoF * UnF * weightL * area + diff_viscous * area +
						// diffPDU*nvec[2] * nvec[2] * area
						// );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( RhoF * WUR * nvec[2] * WF * area + RhoF * UnF * weightR * area - diff_viscous * area -
						// diffPDU*nvec[2] * nvec[2] * area
						// ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			// A_vals[id] -= ( RhoF * WUR * nvec[2] * WF * area + RhoF * UnF * weightR * area - diff_viscous * area -
						// diffPDU*nvec[2] * nvec[2] * area
						// );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[2] * WF * area + RhoF * UnF * weightL * area + diff_viscous * area +
						// diffPDU*nvec[2] * nvec[2] * area
						// ));
        ...
        // // T'
		// id = step_loc_L*(B_n*3+4) + i_loc_L; i_glo = str_glo_L + step_loc_L*3 + i_loc_L; j_glo = str_glo_R + step_loc_R*4 + i_loc_R;
		// A_vals[id] += ( weightL * dRhoDTL * UnF * WF * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDTR * UnF * WF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*3+4) + i_loc_R; i_glo = str_glo_R + step_loc_R*3 + i_loc_R; j_glo = str_glo_L + step_loc_L*4 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDTR * UnF * WF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDTL * UnF * WF * area ));
        ...
        // // MF[0]'
		// id = step_loc_L*(B_n*3+5) + i_loc_L; i_glo = str_glo_L + step_loc_L*3 + i_loc_L; j_glo = str_glo_R + step_loc_R*5 + i_loc_R;
		// A_vals[id] += ( weightL * dRhoDMFL[0] * UnF * WF * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDMFR[0] * UnF * WF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*3+5) + i_loc_R; i_glo = str_glo_R + step_loc_R*3 + i_loc_R; j_glo = str_glo_L + step_loc_L*5 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDMFR[0] * UnF * WF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDMFL[0] * UnF * WF * area ));
        ...
        // // p'
        // id = step_loc_L*(B_n*4+0) + i_loc_L; i_glo = str_glo_L + step_loc_L*4 + i_loc_L; j_glo = str_glo_R + step_loc_R*0 + i_loc_R;
        // A_vals[id] += ( weightL * dRhoDPL * HtF * UnF * area + RhoF * weightL * dHtDPL * UnF * area + RhoF * diffDP * HtF * area );
        // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
        // A_vals.push_back(( weightR * dRhoDPR * HtF * UnF * area + RhoF * weightR * dHtDPR * UnF * area - RhoF * diffDP * HtF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*4+0) + i_loc_R; i_glo = str_glo_R + step_loc_R*4 + i_loc_R; j_glo = str_glo_L + step_loc_L*0 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDPR * HtF * UnF * area + RhoF * weightR * dHtDPR * UnF * area - RhoF * diffDP * HtF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDPL * HtF * UnF * area + RhoF * weightL * dHtDPL * UnF * area + RhoF * diffDP * HtF * area ));
        ...
        // // u'
		// id = step_loc_L*(B_n*4+1) + i_loc_L; i_glo = str_glo_L + step_loc_L*4 + i_loc_L; j_glo = str_glo_R + step_loc_R*1 + i_loc_R;
		// A_vals[id] += ( RhoF * WUL * nvec[0] * HtF * area + RhoF * UnF * weightL * UL * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( RhoF * WUR * nvec[0] * HtF * area + RhoF * UnF * weightR * UR * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*4+1) + i_loc_R; i_glo = str_glo_R + step_loc_R*4 + i_loc_R; j_glo = str_glo_L + step_loc_L*1 + i_loc_L;
			// A_vals[id] -= ( RhoF * WUR * nvec[0] * HtF * area + RhoF * UnF * weightR * UR * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[0] * HtF * area + RhoF * UnF * weightL * UL * area ));
        ...
        // // v'
		// id = step_loc_L*(B_n*4+2) + i_loc_L; i_glo = str_glo_L + step_loc_L*4 + i_loc_L; j_glo = str_glo_R + step_loc_R*2 + i_loc_R;
		// A_vals[id] += ( RhoF * WUL * nvec[1] * HtF * area + RhoF * UnF * weightL * VL * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( RhoF * WUR * nvec[1] * HtF * area + RhoF * UnF * weightR * VR * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*4+2) + i_loc_R; i_glo = str_glo_R + step_loc_R*4 + i_loc_R; j_glo = str_glo_L + step_loc_L*2 + i_loc_L;
			// A_vals[id] -= ( RhoF * WUR * nvec[1] * HtF * area + RhoF * UnF * weightR * VR * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[1] * HtF * area + RhoF * UnF * weightL * VL * area ));
        ...
        // // w'
		// id = step_loc_L*(B_n*4+3) + i_loc_L; i_glo = str_glo_L + step_loc_L*4 + i_loc_L; j_glo = str_glo_R + step_loc_R*3 + i_loc_R;
		// A_vals[id] += ( RhoF * WUL * nvec[2] * HtF * area + RhoF * UnF * weightL * WL * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( RhoF * WUR * nvec[2] * HtF * area + RhoF * UnF * weightR * WR * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*4+3) + i_loc_R; i_glo = str_glo_R + step_loc_R*4 + i_loc_R; j_glo = str_glo_L + step_loc_L*3 + i_loc_L;
			// A_vals[id] -= ( RhoF * WUR * nvec[2] * HtF * area + RhoF * UnF * weightR * WR * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[2] * HtF * area + RhoF * UnF * weightL * WL * area ));
        ...
        // // T'
		// id = step_loc_L*(B_n*4+4) + i_loc_L; i_glo = str_glo_L + step_loc_L*4 + i_loc_L; j_glo = str_glo_R + step_loc_R*4 + i_loc_R;
		// A_vals[id] += ( weightL * dRhoDTL * UnF * HtF * area + RhoF * weightL * dHtDTL * UnF * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDTR * UnF * HtF * area + RhoF * weightR * dHtDTR * UnF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*4+4) + i_loc_R; i_glo = str_glo_R + step_loc_R*4 + i_loc_R; j_glo = str_glo_L + step_loc_L*4 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDTR * UnF * HtF * area + RhoF * weightR * dHtDTR * UnF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDTL * UnF * HtF * area + RhoF * weightL * dHtDTL * UnF * area ));
        ...
        // // MF[0]'
		// id = step_loc_L*(B_n*4+5) + i_loc_L; i_glo = str_glo_L + step_loc_L*4 + i_loc_L; j_glo = str_glo_R + step_loc_R*5 + i_loc_R;
		// A_vals[id] += ( weightL * dRhoDMFL[0] * UnF * HtF * area + RhoF * weightL * dHtDMFL[0] * UnF * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDMFR[0] * UnF * HtF * area + RhoF * weightR * dHtDMFR[0] * UnF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*4+5) + i_loc_R; i_glo = str_glo_R + step_loc_R*4 + i_loc_R; j_glo = str_glo_L + step_loc_L*5 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDMFR[0] * UnF * HtF * area + RhoF * weightR * dHtDMFR[0] * UnF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDMFL[0] * UnF * HtF * area + RhoF * weightL * dHtDMFL[0] * UnF * area ));
        ...
        // // p'
        // id = step_loc_L*(B_n*5+0) + i_loc_L; i_glo = str_glo_L + step_loc_L*5 + i_loc_L; j_glo = str_glo_R + step_loc_R*0 + i_loc_R;
        // A_vals[id] += ( weightL * dRhoDPL * UnF * MFF[0] * area + RhoF * diffDP * MFF[0] * area );
        // A_rows.push_back(i_glo); A_cols.push_back(j_glo);
        // A_vals.push_back(( weightR * dRhoDPR * UnF * MFF[0] * area - RhoF * diffDP * MFF[0] * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*5+0) + i_loc_R; i_glo = str_glo_R + step_loc_R*5 + i_loc_R; j_glo = str_glo_L + step_loc_L*0 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDPR * UnF * MFF[0] * area - RhoF * diffDP * MFF[0] * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDPL * UnF * MFF[0] * area + RhoF * diffDP * MFF[0] * area ));
        ...
        // // u'
		// id = step_loc_L*(B_n*5+1) + i_loc_L; i_glo = str_glo_L + step_loc_L*5 + i_loc_L; j_glo = str_glo_R + step_loc_R*1 + i_loc_R;
		// A_vals[id] += ( RhoF * WUL * nvec[0] * MFF[0] * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( RhoF * WUR * nvec[0] * MFF[0] * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*5+1) + i_loc_R; i_glo = str_glo_R + step_loc_R*5 + i_loc_R; j_glo = str_glo_L + step_loc_L*1 + i_loc_L;
			// A_vals[id] -= ( RhoF * WUR * nvec[0] * MFF[0] * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[0] * MFF[0] * area ));
        ...
        // // v'
		// id = step_loc_L*(B_n*5+2) + i_loc_L; i_glo = str_glo_L + step_loc_L*5 + i_loc_L; j_glo = str_glo_R + step_loc_R*2 + i_loc_R;
		// A_vals[id] += ( RhoF * WUL * nvec[1] * MFF[0] * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( RhoF * WUR * nvec[1] * MFF[0] * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*5+2) + i_loc_R; i_glo = str_glo_R + step_loc_R*5 + i_loc_R; j_glo = str_glo_L + step_loc_L*2 + i_loc_L;
			// A_vals[id] -= ( RhoF * WUR * nvec[1] * MFF[0] * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[1] * MFF[0] * area ));
        ...
        // // w'
		// id = step_loc_L*(B_n*5+3) + i_loc_L; i_glo = str_glo_L + step_loc_L*5 + i_loc_L; j_glo = str_glo_R + step_loc_R*3 + i_loc_R;
		// A_vals[id] += ( RhoF * WUL * nvec[2] * MFF[0] * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( RhoF * WUR * nvec[2] * MFF[0] * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*5+3) + i_loc_R; i_glo = str_glo_R + step_loc_R*5 + i_loc_R; j_glo = str_glo_L + step_loc_L*3 + i_loc_L;
			// A_vals[id] -= ( RhoF * WUR * nvec[2] * MFF[0] * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( RhoF * WUL * nvec[2] * MFF[0] * area ));
        ...
        // // T'
		// id = step_loc_L*(B_n*5+4) + i_loc_L; i_glo = str_glo_L + step_loc_L*5 + i_loc_L; j_glo = str_glo_R + step_loc_R*4 + i_loc_R;
		// A_vals[id] += ( weightL * dRhoDTL * UnF * MFF[0] * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDTR * UnF * MFF[0] * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*5+4) + i_loc_R; i_glo = str_glo_R + step_loc_R*5 + i_loc_R; j_glo = str_glo_L + step_loc_L*4 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDTR * UnF * MFF[0] * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDTL * UnF * MFF[0] * area ));
        ...
        // // MF[0]'
		// id = step_loc_L*(B_n*5+5) + i_loc_L; i_glo = str_glo_L + step_loc_L*5 + i_loc_L; j_glo = str_glo_R + step_loc_R*5 + i_loc_R;
		// A_vals[id] += ( weightL * dRhoDMFL[0] * UnF * MFF[0] * area + RhoF * weightL * UnF * area );
		// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		// A_vals.push_back(( weightR * dRhoDMFR[0] * UnF * MFF[0] * area + RhoF * weightR * UnF * area ));
		...
		// if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			// id = step_loc_R*(B_n*5+5) + i_loc_R; i_glo = str_glo_R + step_loc_R*5 + i_loc_R; j_glo = str_glo_L + step_loc_L*5 + i_loc_L;
			// A_vals[id] -= ( weightR * dRhoDMFR[0] * UnF * MFF[0] * area + RhoF * weightR * UnF * area );
			// A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			// A_vals.push_back(-( weightL * dRhoDMFL[0] * UnF * MFF[0] * area + RhoF * weightL * UnF * area ));
		...
		// vector<double> convective_flux(6,0.0);
		// convective_flux[0] = RhoF_HO * UnF * area;
		// convective_flux[1] = RhoF_HO * UnF * UF * area + PF * nvec[0] * area;
		// convective_flux[2] = RhoF_HO * UnF * VF * area + PF * nvec[1] * area;
		// convective_flux[3] = RhoF_HO * UnF * WF * area + PF * nvec[2] * area;
		// convective_flux[4] = RhoF_HO * UnF * HtF_HO * area;
		// convective_flux[5] = RhoF_HO * UnF * MFF_HO[0] * area;


        // // ----------------------------
		// for(int ii=0; ii<6; ++ii){
			// B[step_loc_L*ii + i_loc_L] -= ( convective_flux[ii] - viscous_flux[ii] );
		// }
		
		// if(face.getType() == SEMO_Types::INTERNAL_FACE){
			// for(int ii=0; ii<6; ++ii){
				// B[step_loc_R*ii + i_loc_R] += ( convective_flux[ii] - viscous_flux[ii] );
			// }
		// }
	...
	// for(auto& boundary : mesh.boundary){
		...
		// if(boundary.neighbProcNo == -1){
			...
			// for(int i=str; i<end; ++i){
				...
				// for(int ii=0; ii<3; ++ii){
					// UnF += coeffP_diff * alpha * tmp1 * gradP[face.owner][ii]*face.unitNomalsPN[ii];
				// }
				...
				// for(int ii=0; ii<3; ++ii){
					...
					// // face gradients, non-orthogonal
					// double tvec = nvec[ii] - alpha * face.unitNomalsPN[ii];
					// dUdnF += gradU[face.owner][ii] * tvec;
					
					// dVdnF += gradV[face.owner][ii] * tvec;
					
					// dWdnF += gradW[face.owner][ii] * tvec;
				// }
				// dUdnF += alpha * (orgUR-orgUL)/dPN;
				// dVdnF += alpha * (orgVR-orgVL)/dPN;
				// dWdnF += alpha * (orgWR-orgWL)/dPN;
				...
				// // // viscous
				// // viscous_flux[1] = muF*(orgUR-orgUL)/dPN*area;
				// // viscous_flux[2] = muF*(orgVR-orgVL)/dPN*area;
				// // viscous_flux[3] = muF*(orgWR-orgWL)/dPN*area;
				
				
					
				// // continuity
				// id = step_loc*(B_n*0+0) + i_loc_L;
				// A_vals[id] += coeffP * ( dRhoDPF * UnF * area );
				// A_vals[id] += coeffP_diff * ( RhoF * tmp1 / dPN * area );
				 
				// id = step_loc*(B_n*0+1) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[0] * area );
				
				// id = step_loc*(B_n*0+2) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[1] * area );
				
				// id = step_loc*(B_n*0+3) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[2] * area );
				
				// id = step_loc*(B_n*0+4) + i_loc_L;
				// A_vals[id] += coeffT *( dRhoDTF * UnF * area );
				
				// id = step_loc*(B_n*0+5) + i_loc_L;
				// A_vals[id] += coeffMF *( dRhoDMFF[0] * UnF * area );
				
				
				
				// // x-momentum
				// id = step_loc*(B_n*1+0) + i_loc_L;
				// A_vals[id] += coeffP * ( dRhoDPF * UnF * UF * area );
				// A_vals[id] += coeffP_diff * ( RhoF * alpha * tmp1 / dPN * UF * area );
				
				// id = step_loc*(B_n*1+1) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[0] * UF * area + RhoF * UnF * area );
				// A_vals[id] += coeffUVW_diff *( alpha * muF / dPN * area );
				
				// // id = step_loc*(B_n*1+2) + i_loc_L;
				// // A_vals[id] += coeffUVW *( RhoF * nvec[1] * UF * area );
				
				// // id = step_loc*(B_n*1+3) + i_loc_L;
				// // A_vals[id] += coeffUVW *( RhoF * nvec[2] * UF * area );
				
				// id = step_loc*(B_n*1+4) + i_loc_L;
				// A_vals[id] += coeffT *( dRhoDTF * UnF * UF * area );
				
				// id = step_loc*(B_n*1+5) + i_loc_L;
				// A_vals[id] += coeffMF *( dRhoDMFF[0] * UnF * UF * area );
				
				
				
				// // y-momentum
				// id = step_loc*(B_n*2+0) + i_loc_L;
				// A_vals[id] += coeffP * ( dRhoDPF * UnF * VF * area );
				// A_vals[id] += coeffP_diff * ( RhoF * alpha * tmp1 / dPN * VF * area );
				
				// // id = step_loc*(B_n*2+1) + i_loc_L;
				// // A_vals[id] += coeffUVW *( RhoF * nvec[0] * VF * area );
				
				// id = step_loc*(B_n*2+2) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[1] * VF * area + RhoF * UnF * area );
				// A_vals[id] += coeffUVW_diff *( alpha * muF / dPN * area );
				
				// // id = step_loc*(B_n*2+3) + i_loc_L;
				// // A_vals[id] += coeffUVW *( RhoF * nvec[2] * VF * area );
				
				// id = step_loc*(B_n*2+4) + i_loc_L;
				// A_vals[id] += coeffT *( dRhoDTF * UnF * VF * area );
				
				// id = step_loc*(B_n*2+5) + i_loc_L;
				// A_vals[id] += coeffMF *( dRhoDMFF[0] * UnF * VF * area );

				
				// // z-momentum
				// id = step_loc*(B_n*3+0) + i_loc_L;
				// A_vals[id] += coeffP * ( dRhoDPF * UnF * WF * area );
				// A_vals[id] += coeffP_diff * ( RhoF * alpha * tmp1 / dPN * WF * area );
				
				// // id = step_loc*(B_n*3+1) + i_loc_L;
				// // A_vals[id] += coeffUVW *( RhoF * nvec[0] * WF * area );
				
				// // id = step_loc*(B_n*3+2) + i_loc_L;
				// // A_vals[id] += coeffUVW *( RhoF * nvec[1] * WF * area );
				
				// id = step_loc*(B_n*3+3) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[2] * WF * area + RhoF * UnF * area );
				// A_vals[id] += coeffUVW_diff *( alpha * muF / dPN * area );
				
				// id = step_loc*(B_n*3+4) + i_loc_L;
				// A_vals[id] += coeffT *( dRhoDTF * UnF * WF * area );
				
				// id = step_loc*(B_n*3+5) + i_loc_L;
				// A_vals[id] += coeffMF *( dRhoDMFF[0] * UnF * WF * area );

				
				// // energy
				// id = step_loc*(B_n*4+0) + i_loc_L;
				// A_vals[id] += coeffP * ( dRhoDPF * UnF * HtF * area + RhoF * UnF * dHtDPF * area );
				// A_vals[id] += coeffP_diff * ( RhoF * alpha * tmp1 / dPN * dHtDPF * area );
				
				// id = step_loc*(B_n*4+1) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[0] * HtF * area + RhoF * UnF * UF * area );
				
				// id = step_loc*(B_n*4+2) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[1] * HtF * area + RhoF * UnF * VF * area );
				
				// id = step_loc*(B_n*4+3) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[2] * HtF * area + RhoF * UnF * WF * area );
				
				// id = step_loc*(B_n*4+4) + i_loc_L;
				// A_vals[id] += coeffT *( dRhoDTF * UnF * HtF * area + RhoF * UnF * dHtDTF * area );
				
				// id = step_loc*(B_n*4+5) + i_loc_L;
				// A_vals[id] += coeffMF *( dRhoDMFF[0] * UnF * HtF * area + RhoF * UnF * dHtDMFF[0] * area );

				
				// // mass-fraction 0
				// id = step_loc*(B_n*5+0) + i_loc_L;
				// A_vals[id] += coeffP * ( dRhoDPF * UnF * MFF[0] * area );
				// A_vals[id] += coeffP_diff * ( RhoF * alpha * tmp1 / dPN * MFF[0] * area );
				
				// id = step_loc*(B_n*5+1) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[0] * MFF[0] * area );
				
				// id = step_loc*(B_n*5+2) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[1] * MFF[0] * area );
				
				// id = step_loc*(B_n*5+3) + i_loc_L;
				// A_vals[id] += coeffUVW *( RhoF * nvec[2] * MFF[0] * area );
				
				// id = step_loc*(B_n*5+4) + i_loc_L;
				// A_vals[id] += coeffT *( dRhoDTF * UnF * MFF[0] * area );
				
				// id = step_loc*(B_n*5+5) + i_loc_L;
				// A_vals[id] += coeffMF *( dRhoDMFF[0] * UnF * MFF[0] * area + RhoF * UnF * area );

				

				// vector<double> convective_flux(6,0.0);
				// convective_flux[0] = RhoF * UnF * area;
				// convective_flux[1] = RhoF * UnF * UF * area + PF * nvec[0] * area;
				// convective_flux[2] = RhoF * UnF * VF * area + PF * nvec[1] * area;
				// convective_flux[3] = RhoF * UnF * WF * area + PF * nvec[2] * area;
				// convective_flux[4] = RhoF * UnF * HtF * area;
				// convective_flux[5] = RhoF * UnF * MFF[0] * area;


				// // ----------------------------
				// for(int ii=0; ii<B_n; ++ii){
					// B[step_loc*ii + i_loc_L] -= ( convective_flux[ii] - viscous_flux[ii] );
				// }
	...
	// // for(int i=0; i<mesh.cells.size(); ++i){
		...
		// // A_cols.clear();
		// // // A_vals.clear();
		// // vector<double> A_vals2;
		
		// // A_rows.resize(36,-1);
		// // A_cols.resize(36,-1);
		// // A_vals2.resize(36,0.0);
		
		// // for(int j=0; j<B_n; ++j){
			// // for(int k=0; k<B_n; ++k){
				// // // A_rows[j*B_n+k] = j; A_cols[j*B_n+k] = k; A_vals2[j*B_n+k] = matrixA[j][k] * cell.volume / controls.timeStep;
				// // id = step_loc*(B_n*j+k) + i_loc_L;
				// // A_rows[j*B_n+k] = j; A_cols[j*B_n+k] = k; A_vals2[j*B_n+k] = A_vals[id];
				
				// // // if( abs(A_vals[id]-matrixA[j][k] * cell.volume / controls.timeStep) > 0.1 ){
					// // // cout << "  j , k = " << j << " " << k << endl;
					// // // cout << A_vals[id] << " " << matrixA[j][k] * cell.volume / controls.timeStep << endl;
				// // // }
			// // }
		// // }
		...
		// // B2.clear();
		// // B2.resize(6,1.0);
		// // B2[0] = B[step_loc*0 + i_loc_L];
		// // B2[1] = B[step_loc*1 + i_loc_L];
		// // B2[2] = B[step_loc*2 + i_loc_L];
		// // B2[3] = B[step_loc*3 + i_loc_L];
		// // B2[4] = B[step_loc*4 + i_loc_L];
		// // B2[5] = B[step_loc*5 + i_loc_L];
		
		// // vector<double> resiVar2;
		// // resiVar2.resize(6,0.0);
		
		// // solveAMGCL("coupled", mesh, B_n, A_rows, A_cols, A_vals2, B2, resiVar2);
		
		// // resiVar[step_loc*0 + i_loc_L] = resiVar2[0];
		// // resiVar[step_loc*1 + i_loc_L] = resiVar2[1];
		// // resiVar[step_loc*2 + i_loc_L] = resiVar2[2];
		// // resiVar[step_loc*3 + i_loc_L] = resiVar2[3];
		// // resiVar[step_loc*4 + i_loc_L] = resiVar2[4];
		// // resiVar[step_loc*5 + i_loc_L] = resiVar2[5];
		...
		// // // for(int j=0; j<B_n; ++j){
			...
			// // // for(int k=0; k<B_n; ++k){
				// // // updateResi += matrixA[j][k]*B[step_loc*k + i_loc_L] * controls.timeStep/cell.volume;
				
			// // // }
			// // // resiVar[step_loc*j + i_loc_L] = updateResi;
		// // // }
	...
	// // double relaxT = 0.3;
	// // double relaxMF = 0.3;
	// double relaxP = controls.momVelURF;
	// double relaxUVW = controls.momVelURF;
	// double relaxT = controls.momVelURF;
	// double relaxMF = controls.momVelURF;
	...
	// for(int i=0; i<mesh.cells.size(); ++i){
		...
		// cell.var[controls.T] +=     relaxT   * resiVar[step_loc*4 + i_loc];
		// cell.var[controls.MF[0]] += relaxMF  * resiVar[step_loc*5 + i_loc];
		
		// cell.var[controls.MF[0]] = max(0.0,min(1.0,cell.var[controls.MF[0]]));
		// // cell.var[controls.MF[0]] = max(1.e-8,min(1.0-1.e-8,cell.var[controls.MF[0]])); 
		...
		// for(int j=0; j<6; ++j){
			// normDelPrim += pow(resiVar[step_loc*j + i_loc],2.0);
		// }
