...
double SEMO_Solvers_Builder::calcCompPressureEq(
	...
	// for(int i=0; i<mesh.faces.size(); ++i){
        ...
        // // ----------------------------
		// B[step_loc_L*0 + i_loc_L] -= ( RhoF * UnF * area );
		
		// if(face.getType() == SEMO_Types::INTERNAL_FACE){
			// B[step_loc_R*0 + i_loc_R] += ( RhoF * UnF * area );
		// }
	...
	// for(auto& boundary : mesh.boundary){
		...
		// if(boundary.neighbProcNo == -1){
			...
			// for(int i=str; i<end; ++i){
				...
				// // ----------------------------
				// B[step_loc*0 + i_loc_L] -= ( RhoF * UnF * area );
