...
double SEMO_Solvers_Builder::calcCompMassfracEq(
	...
	// for(int i=0; i<mesh.cells.size(); ++i){
        ...
        // B[step_loc*0 + i_loc] = -(Rho*MF[0] - oldRho*oldMF[0])*volume / timeStep;
	...
	// for(int i=0; i<mesh.faces.size(); ++i){
        ...
        // // ----------------------------
		// B[step_loc_L*0 + i_loc_L] -= ( RhoF_HO * UnF * MFF_HO[0] * area );
		
		// if(face.getType() == SEMO_Types::INTERNAL_FACE){
			// B[step_loc_R*0 + i_loc_R] += ( RhoF_HO * UnF * MFF_HO[0] * area );
		// }
	...
	// for(auto& boundary : mesh.boundary){
		...
		// if(boundary.neighbProcNo == -1){
			...
			// for(int i=str; i<end; ++i){
				...
				// // ----------------------------
				// B[step_loc*0 + i_loc_L] -= ( RhoF * UnF * MFF[0] * area );
	...
	// for(int i=0; i<mesh.cells.size(); ++i){
		...
		// cell.var[controls.MF[0]] += relaxMF  * resiVar[step_loc*0 + i_loc];
	...
	// if(rank==0){
		...
		// double resi_print = sqrt(normDelPrim);
